# Data Model: 提示词模板管理

**Feature**: 025-prompt-template
**Date**: 2025-12-26

## Entity Relationship Diagram

```
┌─────────────────────┐       ┌──────────────────────────┐
│   template_usage    │       │    prompt_template       │
├─────────────────────┤       ├──────────────────────────┤
│ id (PK)             │◄──────│ usage_id (FK)            │
│ code (UNIQUE)       │   1:N │ id (PK)                  │
│ name                │       │ name (UNIQUE)            │
│ description         │       │ description              │
│ deleted             │       │ current_version          │
│ created_at          │       │ version (乐观锁)          │
│ updated_at          │       │ deleted                  │
└─────────────────────┘       │ created_by               │
                              │ created_at               │
                              │ updated_at               │
                              └──────────────────────────┘
                                          │
                                          │ 1:N
                                          ▼
                              ┌──────────────────────────┐
                              │ prompt_template_version  │
                              ├──────────────────────────┤
                              │ id (PK)                  │
                              │ template_id (FK)         │
                              │ version_number           │
                              │ content (TEXT)           │
                              │ change_note              │
                              │ created_by               │
                              │ created_at               │
                              └──────────────────────────┘
```

## Entities

### 1. TemplateUsage (模板用途)

| Field       | Type         | Constraints              | Description        |
|-------------|--------------|--------------------------|-------------------|
| id          | BIGINT       | PK, AUTO_INCREMENT       | 主键 ID            |
| code        | VARCHAR(50)  | UNIQUE, NOT NULL         | 用途编码（如 FAULT_DIAGNOSIS） |
| name        | VARCHAR(100) | NOT NULL                 | 用途名称（如 故障诊断） |
| description | VARCHAR(500) | NULL                     | 用途描述           |
| deleted     | TINYINT(1)   | NOT NULL, DEFAULT 0      | 逻辑删除标记        |
| created_at  | DATETIME     | NOT NULL                 | 创建时间           |
| updated_at  | DATETIME     | NOT NULL                 | 更新时间           |

**Validation Rules**:
- `code`: 2-50 字符，大写字母和下划线，如 `FAULT_DIAGNOSIS`
- `name`: 1-100 字符

**Preset Values**:
| code              | name     |
|-------------------|----------|
| FAULT_DIAGNOSIS   | 故障诊断  |
| KNOWLEDGE_QA      | 知识问答  |
| CODE_GENERATION   | 代码生成  |
| OPS_SUGGESTION    | 运维建议  |

---

### 2. PromptTemplate (提示词模板)

| Field            | Type         | Constraints              | Description        |
|------------------|--------------|--------------------------|-------------------|
| id               | BIGINT       | PK, AUTO_INCREMENT       | 主键 ID            |
| name             | VARCHAR(200) | UNIQUE, NOT NULL         | 模板名称           |
| usage_id         | BIGINT       | FK -> template_usage.id  | 用途 ID            |
| description      | VARCHAR(1000)| NULL                     | 模板描述           |
| current_version  | INT          | NOT NULL, DEFAULT 1      | 当前版本号         |
| version          | INT          | NOT NULL, DEFAULT 0      | 乐观锁版本（数据库行） |
| deleted          | TINYINT(1)   | NOT NULL, DEFAULT 0      | 逻辑删除标记        |
| created_by       | BIGINT       | NOT NULL                 | 创建人 ID          |
| created_at       | DATETIME     | NOT NULL                 | 创建时间           |
| updated_at       | DATETIME     | NOT NULL                 | 更新时间           |

**Validation Rules**:
- `name`: 1-200 字符，系统内唯一
- `current_version`: >= 1

**Indexes**:
- UNIQUE INDEX `uk_name` ON (`name`) WHERE `deleted` = 0
- INDEX `idx_usage_id` ON (`usage_id`)
- INDEX `idx_deleted` ON (`deleted`)

---

### 3. PromptTemplateVersion (模板版本)

| Field          | Type         | Constraints                  | Description        |
|----------------|--------------|------------------------------|-------------------|
| id             | BIGINT       | PK, AUTO_INCREMENT           | 主键 ID            |
| template_id    | BIGINT       | FK -> prompt_template.id, NOT NULL | 模板 ID       |
| version_number | INT          | NOT NULL                     | 版本号             |
| content        | TEXT         | NOT NULL                     | 模板内容（最大 64KB） |
| change_note    | VARCHAR(500) | NULL                         | 变更说明           |
| created_by     | BIGINT       | NOT NULL                     | 创建人 ID          |
| created_at     | DATETIME     | NOT NULL                     | 创建时间           |

**Validation Rules**:
- `content`: 1-65535 字节（64KB）
- `version_number`: >= 1
- `change_note`: 0-500 字符

**Indexes**:
- UNIQUE INDEX `uk_template_version` ON (`template_id`, `version_number`)
- INDEX `idx_template_created` ON (`template_id`, `created_at` DESC)

---

## State Transitions

### PromptTemplate Lifecycle

```
                   ┌─────────────────────────────────────┐
                   │                                     │
                   ▼                                     │
┌─────────┐    ┌─────────┐    ┌─────────┐    ┌─────────┐│
│ Created │───▶│ Active  │───▶│ Updated │───▶│ Active  ││
└─────────┘    └─────────┘    └─────────┘    └─────────┘│
     │              │              │              │      │
     │              │              │              │      │
     │              ▼              │              │      │
     │         ┌─────────┐        │              │      │
     │         │ Deleted │◄───────┴──────────────┘      │
     │         └─────────┘                              │
     │                                                   │
     └───────────────────────────────────────────────────┘
                         Rollback
```

**State Descriptions**:
- **Created**: 新建模板，version = 1
- **Active**: 正常状态，可读写
- **Updated**: 更新时创建新版本，current_version++
- **Deleted**: 软删除，deleted = 1

---

## Database Migration Script

```sql
-- V025__create_prompt_template_tables.sql

-- 1. 模板用途表
CREATE TABLE IF NOT EXISTS `template_usage` (
    `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '主键ID',
    `code` VARCHAR(50) NOT NULL COMMENT '用途编码',
    `name` VARCHAR(100) NOT NULL COMMENT '用途名称',
    `description` VARCHAR(500) DEFAULT NULL COMMENT '用途描述',
    `deleted` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '逻辑删除: 0-未删除, 1-已删除',
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_code` (`code`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='模板用途表';

-- 预置用途数据
INSERT INTO `template_usage` (`code`, `name`, `description`) VALUES
('FAULT_DIAGNOSIS', '故障诊断', '用于故障分析和诊断场景的提示词模板'),
('KNOWLEDGE_QA', '知识问答', '用于知识库问答场景的提示词模板'),
('CODE_GENERATION', '代码生成', '用于代码生成场景的提示词模板'),
('OPS_SUGGESTION', '运维建议', '用于运维建议和最佳实践场景的提示词模板');

-- 2. 提示词模板表
CREATE TABLE IF NOT EXISTS `prompt_template` (
    `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '主键ID',
    `name` VARCHAR(200) NOT NULL COMMENT '模板名称',
    `usage_id` BIGINT DEFAULT NULL COMMENT '用途ID',
    `description` VARCHAR(1000) DEFAULT NULL COMMENT '模板描述',
    `current_version` INT NOT NULL DEFAULT 1 COMMENT '当前版本号',
    `version` INT NOT NULL DEFAULT 0 COMMENT '乐观锁版本',
    `deleted` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '逻辑删除: 0-未删除, 1-已删除',
    `created_by` BIGINT NOT NULL COMMENT '创建人ID',
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_name` (`name`),
    KEY `idx_usage_id` (`usage_id`),
    KEY `idx_deleted` (`deleted`),
    CONSTRAINT `fk_template_usage` FOREIGN KEY (`usage_id`) REFERENCES `template_usage` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='提示词模板表';

-- 3. 模板版本表
CREATE TABLE IF NOT EXISTS `prompt_template_version` (
    `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '主键ID',
    `template_id` BIGINT NOT NULL COMMENT '模板ID',
    `version_number` INT NOT NULL COMMENT '版本号',
    `content` TEXT NOT NULL COMMENT '模板内容',
    `change_note` VARCHAR(500) DEFAULT NULL COMMENT '变更说明',
    `created_by` BIGINT NOT NULL COMMENT '创建人ID',
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_template_version` (`template_id`, `version_number`),
    KEY `idx_template_created` (`template_id`, `created_at` DESC),
    CONSTRAINT `fk_version_template` FOREIGN KEY (`template_id`) REFERENCES `prompt_template` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='模板版本表';
```
