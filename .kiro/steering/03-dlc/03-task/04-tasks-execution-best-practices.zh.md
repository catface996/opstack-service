...
| 第一步 | 理解任务 | 任务理解确认 | ✅ 理解完成 |
| 第二步 | 实现功能 | 工作代码 | ✅ 构建成功 |
| 第三步 | 验证任务 | 验证结果 | ✅ 验收通过 |
| 第四步 | 任务完成确认 | 完成确认 | ✅ 质量达标 |
| 第五步 | 一致性检查 | 一致性报告 | ✅ 与需求设计一致 |
...
- [ ] 已确认前置条件是否满足？
- [ ] 已识别可能的技术难点？
- [ ] 已理解任务的验证方式（运行时/构建/静态）？
- [ ] 已确认任务关联的需求编号？

**If 检查未通过**: **STOP**，先补充对任务的理解或确认前置条件。

**MUST DO**:
- 你 MUST 在完成任务后立即运行构建命令
- 你 MUST 确保构建成功后再进入下一个任务
- 你 MUST 按照验证优先级进行验证（运行时>构建>静态）
- 你 MUST 在任务验证通过后检查与需求设计的一致性

**NEVER DO**:
- NEVER 在构建失败的情况下继续下一个任务
- NEVER 跳过验证直接标记任务完成
- NEVER 预先声明尚未创建的模块或组件
- NEVER 忽略需求和设计的一致性检查
