...
|------|------|------|------|
| **语言** | Java | 21 (LTS) | ✅ 已采用 |
| **框架** | Spring Boot | 3.4.1 | ✅ 已采用 |
| **数据库** | MySQL | 8.0+ | ✅ 已采用 |
| **缓存** | Redis | 7.0+ | ✅ 已采用 |
| **消息队列** | AWS SQS | 2.20.0 | ✅ 已采用 |
| **安全** | JWT + BCrypt | - | ✅ 已采用 |
...
**MUST 严格遵守以下依赖规则，违反将导致架构腐化：**

| 规则 | 描述 | 状态 |
|------|------|------|
| ✅ MUST | Application Service 只能调用 Domain Service | 强制 |
| ✅ MUST | Domain Service 是数据访问的唯一入口 | 强制 |
| ✅ MUST | Repository/Cache/MQ 接口定义在 Domain 层 | 强制 |
| ✅ MUST | Infrastructure 层实现 Domain 层定义的接口 | 强制 |
| ❌ NEVER | Application Service 禁止直接调用 Repository/Cache/MQ | 强制 |
...
## 技术选型原则 (NON-NEGOTIABLE)

**MUST 遵守以下选型原则，NEVER 因个人偏好选择技术：**

| 优先级 | 原则 | 描述 | 检查方式 |
|--------|------|------|----------|
| P0 | **成熟稳定** | 社区活跃、生产验证 | GitHub Stars > 10k |
| P0 | **性能优先** | 满足性能指标 | 压测验证 |
| P1 | **易于维护** | 文档齐全、学习曲线平缓 | 团队评估 |
| P1 | **生态完善** | 插件丰富、集成方便 | 生态调研 |
| P2 | **成本可控** | 学习成本、运维成本可接受 | 成本分析 |
